...
ENDEND:
  ...
  */
  ...
  /* libretro: present here too — YuiSwapBuffers() drives VIDVulkanSwapBuffers()
     -> set_image() (the HW-context present path). Without this, frames render to
     the output image but are never handed to the frontend = black screen. */
  YuiSwapBuffers();
  fbRender->onEndFrame();
...
#ifdef __LIBRETRO__
      /* libretro owns aspect/scaling via geometry.aspect_ratio. Do NOT letterbox
         or center internally: that leaves originx/originy non-zero and shrinks
         renderWidth/Height, which both shifts the native viewport AND offsets the
         upscale blit destination (viewportData), truncating the image on the
         frontend. Render the full frame into the output image at origin (0,0). */
      if (false) {
#else
...
void VIDVulkan::generateSubRenderTarget(int width, int height) {
...
#ifdef __LIBRETRO__
  /* The libretro window render pass (vidvulkan_bridge LibretroWindow::_rp) is a
     single B8G8R8A8 color attachment with no depth. VDP2 layer pipelines are
     built against that pass (setRenderPath(getRenderPass())), so for them to be
     usable when rendering into this sub render target (the upscale path), this
     pass must be render-pass-COMPATIBLE: same single BGRA color attachment, no
     depth. Otherwise the layers silently fail to draw and the blit copies black. */
  const VkFormat subColorFormat = VK_FORMAT_B8G8R8A8_UNORM;
#else
  const VkFormat subColorFormat = VK_FORMAT_R8G8B8A8_UNORM;
#endif

  // Color attachment
  VkImageCreateInfo image = vks::initializers::imageCreateInfo();
  image.imageType = VK_IMAGE_TYPE_2D;
  image.format = subColorFormat;
  ...
  colorImageView.viewType = VK_IMAGE_VIEW_TYPE_2D;
  colorImageView.format = subColorFormat;
  colorImageView.subresourceRange = {};
  colorImageView.subresourceRange.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
  colorImageView.subresourceRange.baseMipLevel = 0;
  colorImageView.subresourceRange.levelCount = 1;
  ...
  std::array<VkAttachmentDescription, 2> attchmentDescriptions = {};
  // Color attachment
  attchmentDescriptions[0].format = subColorFormat;
  attchmentDescriptions[0].samples = VK_SAMPLE_COUNT_1_BIT;
  attchmentDescriptions[0].loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;
  attchmentDescriptions[0].storeOp = VK_ATTACHMENT_STORE_OP_STORE;
  attchmentDescriptions[0].stencilLoadOp = VK_ATTACHMENT_LOAD_OP_DONT_CARE;
  ...
  subpassDescription.pColorAttachments = &colorReference;
#ifdef __LIBRETRO__
  /* No depth: keep this pass render-pass-compatible with the color-only window
     render pass the VDP2 pipelines were built against (else layers don't draw). */
  ...
  renderPassInfo.sType = VK_STRUCTURE_TYPE_RENDER_PASS_CREATE_INFO;
#ifdef __LIBRETRO__
  /* Color-only: single attachment, matching the window render pass. */
...
#ifdef __LIBRETRO__
  fbufCreateInfo.attachmentCount = 1; /* color only — matches the color-only pass */
#else
  fbufCreateInfo.attachmentCount = 2;
#endif
  fbufCreateInfo.pAttachments = attachments;
...
void VIDVulkan::blitSubRenderTarget(VkCommandBuffer commandBuffer, const glm::vec4 &viewportData) {
...
#ifdef __LIBRETRO__
  /* libretro HW context: no swapchain. Destination (_out_img) must end in
     SHADER_READ_ONLY_OPTIMAL to match set_image(); subRenderTarget lives in
     SHADER_READ_ONLY_OPTIMAL (its render-pass finalLayout), not PRESENT_SRC_KHR. */
  const VkImageLayout subSrcLayout  = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
  const VkImageLayout dstFinalLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
#else
  const VkImageLayout subSrcLayout  = VK_IMAGE_LAYOUT_PRESENT_SRC_KHR;
  const VkImageLayout dstFinalLayout = VK_IMAGE_LAYOUT_GENERAL;
#endif
  ...
  // Transition the source (sub render target) to transfer-source layout
  vks::tools::insertImageMemoryBarrier(
      commandBuffer, subRenderTarget.color.image, VK_ACCESS_MEMORY_READ_BIT, VK_ACCESS_TRANSFER_READ_BIT,
      subSrcLayout, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, VK_PIPELINE_STAGE_TRANSFER_BIT,
  ...
  // Transition destination image to its final presentation layout
  vks::tools::insertImageMemoryBarrier(commandBuffer, dstImage, VK_ACCESS_TRANSFER_WRITE_BIT, VK_ACCESS_MEMORY_READ_BIT,
                                       VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, dstFinalLayout,
                                       VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT,
                                       VkImageSubresourceRange{VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0, 1});

  // Transition back the sub render target after the blit is done
  vks::tools::insertImageMemoryBarrier(
      commandBuffer, subRenderTarget.color.image, VK_ACCESS_TRANSFER_READ_BIT, VK_ACCESS_MEMORY_READ_BIT,
      VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, subSrcLayout, VK_PIPELINE_STAGE_TRANSFER_BIT,
      VK_PIPELINE_STAGE_TRANSFER_BIT, VkImageSubresourceRange{VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0, 1});
}
