...
Window::Window(Renderer* renderer, VkRenderPass rp, VkFramebuffer fb, VkImage img, uint32_t w, uint32_t h)
  : _renderer(renderer), _surface_size_x(w), _surface_size_y(h),
    _render_pass(rp), _render_pass_keep(rp)
{
  _framebuffers.push_back(fb);
  _active_swapchain_image_id = 0;
  _swapchain_image_count = 1;
...
/* Stub OS-specific functions for libretro (provided by platform-specific files otherwise) */
#ifdef __LIBRETRO__
