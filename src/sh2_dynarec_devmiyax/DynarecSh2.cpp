...
/* Linux/GCC SysV x86_64 JIT fragment sizes, measured from dynalib_x86_64.asm.o:
   nm -n gives prologue=0x00 seperator_normal=0x2a seperator_delay_slot=0x35
   seperator_delay_after=0x60 epilogue=0x78 PageFlip=0x85 seperator_d_normal=0xa2
   seperator_d_delay=0xcd. These differ from the _WIN64 values (Win64 prologue uses
   shadow space). Without this branch the build fell through to the ARMv7 sizes and
   emitted truncated/misaligned blocks -> bogus PC -> crash in CompileBlock. */
#define PROLOGSIZE		     (0x2a)          /* 42 */
#define SEPERATORSIZE_NORMAL (0x35-0x2a)     /* 11 */
#define SEPERATORSIZE_DELAY_SLOT  (0x60-0x35)/* 43 */
#define SEPERATORSIZE_DELAY_AFTER (0x78-0x60)/* 24 */
#define EPILOGSIZE		      (0x85-0x78)    /* 13 */
#define DELAYJUMPSIZE	     (0xa2-0x85)     /* 29 (PageFlip) */
#define SEPERATORSIZE_DEBUG  (0xcd-0xa2)     /* 43 */
#define SEPERATORSIZE_DELAYD_DEBUG (55)
#define NORMAL_CLOCK_OFFSET 10
#define DALAY_CLOCK_OFFSET 10
#define NORMAL_CLOCK_OFFSET_DEBUG 5
#define DALAY_CLOCK_OFFSET_DEBUG 10
...
Block * CompileBlocks::CompileBlock(u32 pc, addrs * ParentT = NULL)
{
  ...
  /* x86_64 idle-skip detection: a pure "DT Rn; BF <self>" countdown delay loop.
     x86_64 has no internal_jmp, so such loops would otherwise run every iteration
     through the C++ dispatcher (catastrophically slow on multi-million-iter BIOS
     delays). Detect the pattern so Execute() can fast-forward it like sh2idle. */
  {
    Block *blk = &g_CompleBlock[blockCount];
    blk->delayReg = 0xFF;
    static int noidle = getenv("YABA_NOIDLE") ? 1 : 0;
    u16 o0 = noidle ? 0 : MappedMemoryReadInst(pc, NULL);
    u16 o1 = MappedMemoryReadInst(pc + 2, NULL);
    if ((o0 & 0xF0FF) == 0x4010 && (o1 & 0xFF00) == 0x8B00) { /* DT Rn ; BF disp */
      s32 disp = (s8)(o1 & 0xFF);
      u32 tgt = (pc + 2) + 4 + (disp << 1);
      if (tgt == pc) { /* branches back to the DT => self countdown loop */
        blk->delayReg = (o0 >> 8) & 0xF;
        blk->delayExitPC = pc + 4;
        blk->delayPerIter = asm_list[opcodeIndex(o0)].cycle + asm_list[opcodeIndex(o1)].cycle;
        if (blk->delayPerIter == 0) blk->delayPerIter = 1;
      }
    }
  }
...
#if defined(_WINDOWS) || defined(X86_64) || defined(__x86_64__)
  /* x86_64 (Windows AND Linux/SysV): off3 is the 16-bit immediate of `mov ax,<disp>`
     — patch the full 12-bit displacement as one u16. The ARM path below splits it
     into two bytes at off3 and off3+4, which corrupts x86_64 branch templates. */
  if (*(op->off3) != 0xFF)
...
int CompileBlocks::overrideMemFunc( void* ptr, int i ) {
  ...
  u8 * override_func = (u8*)malloc(sizeof(u8)*copysize+8);
  memcpy((void*)override_func, (void*)(asm_list[i].func), copysize);

  /* The x86_64 templates embed cache-path accessors as full 64-bit movabs
     immediates (`mov rax, memGetByte`). When SH2 cache emulation is off we
     swap them for the NoCache variants. Match and patch the FULL 8-byte
     immediate: a 32-bit compare/patch breaks once the core .so loads above
     4 GB (ASLR), which silently disabled this override on x86_64. */
  static const uintptr_t from[6] = {
    (uintptr_t)memSetByte, (uintptr_t)memSetWord, (uintptr_t)memSetLong,
    (uintptr_t)memGetByte, (uintptr_t)memGetWord, (uintptr_t)memGetLong,
  };
  static const uintptr_t to[6] = {
    (uintptr_t)memSetByteNoCache, (uintptr_t)memSetWordNoCache, (uintptr_t)memSetLongNoCache,
    (uintptr_t)memGetByteNoCache, (uintptr_t)memGetWordNoCache, (uintptr_t)memGetLongNoCache,
  };
  int hitcount = 0;
  for (int funi = 0; funi + 8 <= copysize; funi++) {
    uintptr_t v;
    memcpy(&v, &override_func[funi], 8);
    for (int k = 0; k < 6; k++) {
      if (v == from[k]) {
        memcpy(&override_func[funi], &to[k], 8);
        hitcount++;
        funi += 7;
        break;
      }
...
int CompileBlocks::EmmitCode(Block *page, addrs * ParentT )
{
  ...
  while (1) {
    ...
    static int block1 = getenv("YABA_BLOCK1") ? 1 : 0;
    if (block1 && count >= 1 && start_addr >= 0x06010000 && start_addr < 0x06013200) {
      break; // debug: force 1-instruction blocks in the window for per-instruction trace
    }
...
//  }
  ...
  /* Fast-forward a "DT Rn; BF self" pure delay loop instead of dispatching every
     iteration (x86_64 has no internal_jmp). Matches sh2idle: decrement Rn within the
     remaining cycle budget; complete (Rn=0,T=1,exit) or carry over to next frame. */
  if (pBlock->delayReg != 0xFF) {
    u32 n = pBlock->delayReg;
    u32 *g = GetGenRegPtr();
    u32 Rn = g[n];
    u32 cur = GET_COUNT();
    u32 budget = (m_pDynaSh2->exitcount > cur) ? (m_pDynaSh2->exitcount - cur) : 0;
    u32 per = pBlock->delayPerIter;
    u32 maxit = budget / per;
    if (maxit == 0) maxit = 1; /* always make progress */
    if (Rn != 0 && Rn <= maxit) {
      g[n] = 0;
      m_pDynaSh2->CtrlReg[0] |= 1;            /* T = 1 (Rn hit 0) */
      SET_COUNT(cur + Rn * per);
      SET_PC(pBlock->delayExitPC);
    } else {
      g[n] = Rn - maxit;                       /* still counting (wraps for Rn==0 edge) */
      m_pDynaSh2->CtrlReg[0] &= ~1u;           /* T = 0 (still looping) */
      SET_COUNT(cur + maxit * per);
      SET_PC(pBlock->b_addr);                  /* resume the loop next frame */
    }
    return 0;
  }
