...
//****************************************************
// Calling convention for the hand-written x86_64 JIT (dynalib_x86_64.asm).
// That asm uses the Microsoft x64 ABI (args in rcx/rdx, rdi/rsi nonvolatile,
// 32-byte shadow space). On Linux SysV we must emit ms_abi versions of the
// block entry (dynaFunc) and every C callback the JIT calls, so the
// conventions match. No-op on Windows / non-x86_64.
//****************************************************
#if defined(__x86_64__) && !defined(_WIN32) && !defined(_WIN64)
  #define DYNAREC_MSABI __attribute__((ms_abi))
#else
  #define DYNAREC_MSABI
#endif
...
struct Block
{
  ...
  u32 id;
  u32 flags;
  /* x86_64 idle-skip: pure "DT Rn; BF/BT self" delay loop fast-forward.
     delayReg = register decremented (0xFF if this block is not such a loop). */
  u32 delayReg;
  u32 delayExitPC; // PC to resume at once the loop completes
  u32 delayPerIter; // SH2 cycles consumed per loop iteration
};
...
typedef void (DYNAREC_MSABI *dynaFunc)(tagSH2 *);
...
extern "C"
{
  DYNAREC_MSABI int EachClock();
  DYNAREC_MSABI int DelayEachClock();
  DYNAREC_MSABI int DebugEachClock();
  DYNAREC_MSABI int DebugDelayClock();

  DYNAREC_MSABI void memSetByte(u32, u8);
  DYNAREC_MSABI void memSetWord(u32, u16);
  DYNAREC_MSABI void memSetLong(u32, u32);

  DYNAREC_MSABI u8 memGetByte(u32);
  DYNAREC_MSABI u16 memGetWord(u32);
  DYNAREC_MSABI u32 memGetLong(u32);

  DYNAREC_MSABI void memSetByteNoCache(u32, u8);
  DYNAREC_MSABI void memSetWordNoCache(u32, u16);
  DYNAREC_MSABI void memSetLongNoCache(u32, u32);

  DYNAREC_MSABI u8 memGetByteNoCache(u32);
  DYNAREC_MSABI u16 memGetWordNoCache(u32);
  DYNAREC_MSABI u32 memGetLongNoCache(u32);
