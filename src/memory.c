...
int YabLoadStateStream(FILE *fp)
{
  ...
  yabsys.frame_count = 0;
   ...
   // Make sure the buffer holds at least the recorded payload. The frontend
   // (libretro) may hand us a fixed-size buffer larger than the actual state
   // with trailing padding, so accept >= rather than requiring an exact match.
   // The recorded `size` is authoritative; chunks are parsed by their own
   // headers, so trailing padding is never read.
   fseek(fp, 0, SEEK_END);
